...
namespace iSHOPNew.Models
{
    ...
    public class SARReport
    {
        ...
        public async Task<string> PrepareSARReport(string filepath, List<string> spNames, HttpContextBase context, FilterPanelInfo filter)
        {
            ...
            int pSlideId = 5;
            var obj = new object();
            DataSet dsetP = new DataSet();
            int pSpIncr = 0;
            
            /*Parallel.ForEach(spNames, (spName) =>
                 {
                     lock (obj)
                     {
                         pSlideId++;
                     ...
                     {
                         var result = GetTableOutputPPT(filter, spName, isCustomName[slideId], slideId, context);
                         if (result != null)
                         {
                             lock (dsetP)
                             {
                                 result.TableName = result.TableName + spName + pSpIncr;
                                 System.Data.DataTable copy = result.Copy();
                                 dsetP.Tables.Add(copy);
                             }
                         }
                     }
                 });*/
            ...
            /*Task<int> t = Task.Run(async () =>
            {
                foreach (var spName in spNames)
                {
                    ...
                    if (!string.IsNullOrEmpty(spName))
                    {
                        var result = await Task.Run(() => GetTableOutputPPT(filter, spName, isCustomName[pSlideId], pSlideId, context));
                        if (result != null)
                        {
                            lock (dsetP)
                            {
            ...
            Task<int> t = Task.Run(async () =>
            {
                Parallel.ForEach(spNames, (spName) =>
                {
                    ...
                    if (!string.IsNullOrEmpty(spName))
                    {
                        var result = GetTableOutputPPT(filter, spName, isCustomName[pSlideId], pSlideId, context);
                        if (result != null)
                        {
                            lock (dsetP)
                            {
