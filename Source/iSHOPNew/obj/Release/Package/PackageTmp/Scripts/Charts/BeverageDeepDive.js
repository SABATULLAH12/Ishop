...
function prepareContentArea() { 
    ...
    debugger;
    if (!Validate_CompareBeverages_Charts() || !Validate_Group_Charts() || !Validate_Measures_Charts()) {
        return false;
    }
    ...
    if (ChartType.toLowerCase() == "table") {       
        ...
        if (ModuleBlock == "PIT") {
            ...
            //if (!Validate_CompareRetailers() || !Validate_Group_Charts() || !Validate_Measures_Charts()) {
            //    return false;
            //}
            $("#RightPanelPartial").show();
            ...
            for (var i = 0; i < Grouplist.length; i++) {
                _profilerparams.Comparison_DBNames.push(Grouplist[i].Name);
                ...
                //if (i == 0 && _profilerparams.Sigtype_UniqueId=="1")
                //    _profilerparams.Benchmark = Grouplist[i].Name;
            }
        }
        else {
            //if (!Validate_CompareRetailers() || !Validate_Measures_Charts()) {
            //    return false;
            //}
            $("#RightPanelPartial").show();
        ...
        if (ComparisonBevlist.length > 0) {
            ...
            if (TabType.toLocaleLowerCase() == "trips")
                _profilerparams.ShopperSegment = ComparisonBevlist[0].LevelDesc.split("||")[0] + "||" + ComparisonBevlist[0].Name;
            else
                _profilerparams.ShopperSegment = ComparisonBevlist[0].LevelDesc.split("||")[0] + "||" + ComparisonBevlist[0].Name;

            if (ModuleBlock != "PIT")
                _profilerparams.Comparison_UniqueIds.push(ComparisonBevlist[0].UniqueId);
            else
                _profilerparams.SingleSelection = ComparisonBevlist[0].UniqueId;
        ...
        if (ComparisonBevlist.length > 0) {
            ...
            _profilerparams.SingleSelection = ComparisonBevlist[0].Name;
            if (ModuleBlock != "PIT")
                _profilerparams.Comparison_UniqueIds.push(ComparisonBevlist[0].UniqueId);
            else
                _profilerparams.SingleSelection = ComparisonBevlist[0].UniqueId;
