...
function prepareContentArea(excelStatus) {
    ...
    if (!Validate_CompareRetailers_Charts() || !Validate_Group_AdvanceAnalysis() || !Validate_Measures_AdvanceAnlysis()) {
        return false;
    }
    ...
    for (var i = 0; i < Grouplist.length; i++) {
        ...
        sCompList.push(Grouplist[i].parentName + " |" + Grouplist[i].Name);
        _profilerparams.Comparison_ShortNames.push(Grouplist[i].Name);
        _profilerparams.Comparison_Ids.push(Grouplist[i].UniqueId);
        sGroupList.push(Grouplist[i].UniqueId)
    }

    _profilerparams.GroupUniqueId = sGroupList.join("|").toString();
    ...
    var sCompListForRetailer = [];
    var Comparison_UniqueIds = [];
    if (TabType.toLocaleLowerCase() == "trips") {
        _profilerparams.Comparison_DBNames.push(Comparisonlist[0].TripsDName);
        sCompListForRetailer.push(Comparisonlist[0].Name);
        _profilerparams.Comparison_UniqueIds = Comparisonlist[0].UniqueId;
    }
    else {
        _profilerparams.Comparison_DBNames.push(Comparisonlist[0].ShopperDBName);
        sCompListForRetailer.push(Comparisonlist[0].Name);
        _profilerparams.Comparison_UniqueIds = Comparisonlist[0].UniqueId;
    }
    _profilerparams.ShopperSegment = sCompListForRetailer.toString();
    _profilerparams.FilterShortnames = Advanced_Filters_ShortNames.join("|");
    _profilerparams.ModuleBlock = "WithinShopper";
    _profilerparams.View = "WithinShopper";
    ...
    _profilerparams.StoreidItems = sGroupList;
...
function PlotTable(leftheader, leftbody, rightheader, rightbody, _width, _fixedColumnWidth, _fixedTableheight) {
    ...
    htmlstring += "<td valign=\"top\" class=\"fixedColumn\" style=\"width:603px;margin-right:10px;\">";
    ...
    htmlstring += "<td valign=\"top\" class=\"fixedContainer\" style=\"width:calc(100% - 629px);\">";
    ...
    if (sNumber > Grouplist.length) {
        var SetWidth = (swidth / (Grouplist.length));
        var SetWidth1 = (SetWidth * (202 / 215));
        //var SetWidth2 = (SetWidth * (244 / 215));
        $(".ColumnWidthHeader").css("width", SetWidth1);
        if (Grouplist.length == 4)
            $(".ColumnWidth").css("width", "150px");
        else
           $(".ColumnWidth").css("width", SetWidth1);
    }
    else {
        var SetWidth = 150;
        $(".ColumnWidth").css("width", SetWidth);
    }
    SetStyles();
    SetScroll($("#contentscroll"), "#393939", 0, -8, 0, -8, 8);
