...
var isCustomBasePopUpVisible = false;
...
$(document).ready(function () {
    $(document).on("click", ".adv-fltr-label", function () {
        ...
        if (currentpage.indexOf("commerce") > -1) {
            if (tabname == "ONLINE ORDERS") {
                LoadTripsFrequencyFilter(sFilterData);
                TabType = "trips";
                $("#RightPanelPartial #visit_frequency_containerId ul li[name='ONLINE ORDER']").removeClass("Selected");
                $("#RightPanelPartial #visit_frequency_containerId ul li[name='ONLINE ORDER']").trigger("click");
                PrepareSearch("ordertype", "Search-ordertypeFilters", "ordertypeFilter-Search-Content", TripsFrequency);
            }
            if (tabname == "ITEMS PURCHASED") {
                LoadTripsFrequencyFilter(sFilterData);
                TabType = "trips";
                //$("#RightPanelPartial #visit_frequency_containerId ul li[name='ONLINE ORDER']").removeClass("Selected");
                //$("#RightPanelPartial #visit_frequency_containerId ul li[name='ONLINE ORDER']").trigger("click");
            ...
            else if (tabname == "AUTO REPLENISHED DELIVERIES") {
                LoadTripsFrequencyFilter(sFilterData);
                TabType = "trips";
                $("#RightPanelPartial #visit_frequency_containerId ul li[name='AUTO-REPLENISHMENT']").removeClass("Selected");
                $("#RightPanelPartial #visit_frequency_containerId ul li[name='AUTO-REPLENISHMENT']").trigger("click");
                PrepareSearch("ordertype", "Search-ordertypeFilters", "ordertypeFilter-Search-Content", TripsFrequency);
            }
            else if (tabname == "ITEMS PURCHASED" || tabname == "DEMOGRAPHICS") {
                LoadTripsFrequencyFilter(sFilterData);
                $("#RightPanelPartial #visit_frequency_containerId ul li[name='TOTAL']").removeClass("Selected");
                $("#RightPanelPartial #visit_frequency_containerId ul li[name='TOTAL']").trigger("click");
                PrepareSearch("ordertype", "Search-ordertypeFilters", "ordertypeFilter-Search-Content", TripsFrequency);
            }
            else { LoadTripsFrequencyFilter(sFilterData); }
        }
            ...
            if ($(this).parent().parent().hasClass("demo")) {
                if (TabType == "trips" && $("#guest-visit-toggle").is(":checked") == true) {
                    ...
                    if (currentpage.indexOf("beverage") > -1) {
                        ...
                        $("#RightPanelPartial #frequency_containerId_trips ul li[name='ALL MONTHLY +']").removeClass("Selected");
                    ...
                    if (currentpage.indexOf("beverage") > -1) {
                        selectedChannels = [];
                        SelectedFrequencyList = [];
                        $("#RightPanelPartial .rgt-cntrl-chnl #rgt-cntrl-chnl-SubFilter1 ul li[name='TOTAL']").removeClass("Selected");
                        $("#RightPanelPartial .rgt-cntrl-chnl #rgt-cntrl-chnl-SubFilter1 ul li[name='TOTAL']").trigger("click");
                    }
                    else {
                        selectedChannels = [];
                        SelectedFrequencyList = [];
...
function GetExportToExcelFrequency(tabitem)
{  
    var exportexcelfre = new Object();
    exportexcelfre = { ShopperFrequency: "", ShopperFrequency_UniqueId: "" };
    var freobj = new Object();
        if (tabitem.TabType == "trips") {
            if (tabitem.tabname.toLocaleLowerCase() == "online orders" || tabitem.tabname.toLocaleLowerCase().indexOf("online ordering") > -1) {
                exportexcelfre.ShopperFrequency = "ONLINE ORDER";
                exportexcelfre.ShopperFrequency_UniqueId = 2;
                return exportexcelfre;
            }           
            else if (tabitem.tabname.toLocaleLowerCase().indexOf("auto replenished") > -1) {
                exportexcelfre.ShopperFrequency = "AUTO-REPLENISHMENT";
                exportexcelfre.ShopperFrequency_UniqueId = 1;
                return exportexcelfre;
            }
            else {
                if (SelectedFrequencyList.length > 0 && SelectedFrequencyList[0].TabType == "trips") {
                    exportexcelfre.ShopperFrequency = SelectedFrequencyList[0].Name;
                    exportexcelfre.ShopperFrequency_UniqueId = SelectedFrequencyList[0].UniqueId;
                }
                else {
                    exportexcelfre.ShopperFrequency = "TOTAL";
                    exportexcelfre.ShopperFrequency_UniqueId = 3;
                }
                return exportexcelfre;
            }
        }
        else if (tabitem.TabType == "shopper") {           
            if (SelectedFrequencyList.length > 0 && SelectedFrequencyList[0].TabType == "shopper") {
                exportexcelfre.ShopperFrequency = SelectedFrequencyList[0].Name;
                exportexcelfre.ShopperFrequency_UniqueId = SelectedFrequencyList[0].UniqueId;
            }
            else {
                //freobj = $("#RightPanelPartial #shopper_frequency_containerId ul li[name='P12M']");
                freobj = $("#RightPanelPartial #shopper_frequency_containerId ul li[name='MONTHLY']");
            }
        }
        if (freobj.length > 0)
        {
            exportexcelfre.ShopperFrequency = $(freobj).children("span").attr("name");
            exportexcelfre.ShopperFrequency_UniqueId = $(freobj).children("span").attr("uniqueid");
        }
    return exportexcelfre;
}
...
function NoDataAvailable() {
    var table = "<div style=\"width:99%;height:20%;text-align:center;border:1px solid grey;margin:auto;\"><span class=\"no-data\">No data available</span></div>";
    $("#Table-Content").html("");
    ...
    $(demo_contentid + ", " + guests_contentid + ", " + visits_contentid).each(function () {
        if ($(this).is(':visible')) {
            ...
            if ($(this).children("span").attr("guest-spname") != "" && $(this).children("span").attr("guest-spname") != undefined) {
                html += "<li><input class=\"exporttoexcel\" type=\"checkbox\" tabtype=\"trips\" tabindex=\"1\" id-mapping=\"" + $(this).children("span").attr("visit-id-mapping") + "\" spname=\"" + $(this).children("span").attr("visit-spname") + "\" samplesize-spname=\"" + $(this).children("span").attr("visit-samplesize-spname") + "\" onclick=\"CheckSelectOrUnSelectSheets();\"><span  spname=\"" + $(this).children("span").attr("visit-spname") + "\" samplesize-spname=\"" + $(this).children("span").attr("visit-samplesize-spname") + "\">ONLINE PURCHASE - " + $(this).children("span").html().toUpperCase() + "</span></li>";
