...
function prepareContentArea() {   
    ...
    if (!Validate_CompareSites() || !Validate_Measures_Charts()) {
        return false;
    }
    if (ModuleBlock == "PIT") {
        if (!Validate_Group_Charts() || !Validate_Group()) {
            return false;
        }
    }
    ...
    if (ChartType.toLowerCase() == "table") {
        ...
        if (ModuleBlock == "PIT") {
            ...
            if (!Validate_CompareSites() || !Validate_Group_Charts() || !Validate_Measures_Charts()) {
                return false;
            }
            $("#RightPanelPartial").show();
            ...
            if (!Validate_CompareSites() || !Validate_Measures_Charts() || !Validate_Trend()) {
                return false;
            }
            $("#RightPanelPartial").show();
        ...
        if (Sites.length > 0) {
            ...
            if (TabType.toLocaleLowerCase() == "trips")
                _profilerparams.ShopperSegment = Sites[0].Name;
            else
                _profilerparams.ShopperSegment = Sites[0].Name;
            ...
            if (ModuleBlock != "PIT") {
                ...
                for (var i = 0; i < Sites.length; i++) {
                    ...
                    sComparisonNames.push(Sites[i].Name);
                }
                _profilerparams.ComparisonNames = sComparisonNames;
            }
            else
                _profilerparams.SingleSelection = Sites[0].UniqueId;
            ...
            if (_profilerparams.Comparison_UniqueIds == null || _profilerparams.Comparison_UniqueIds.length <= 0 || _profilerparams.Comparison_UniqueIds == [])
                _profilerparams.Comparison_UniqueIds.push(Sites[0].UniqueId);
        ...
        if (Sites.length > 0) {
            ...
            _profilerparams.SingleSelection = Sites[0].Name;
            if (ModuleBlock != "PIT") {
                //_profilerparams.Comparison_UniqueIds.push(Comparisonlist[0].UniqueId);
                for (var i = 0; i < Sites.length; i++)
                    _profilerparams.Comparison_UniqueIds.push(Sites[i].UniqueId);
